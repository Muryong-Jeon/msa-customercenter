...
public class MyPageViewHandler {
    ...
    public void whenOrderMaked_then_CREATE_1 (@Payload OrderMaked orderMaked) {
        try {
            if (orderMaked.isMe()) {
                // view 객체 생성
                MyPage myPage = new MyPage();
                // view 객체에 이벤트의 Value 를 set 함
                myPage.setOrderId(orderMaked.getId());
                myPage.setQty(orderMaked.getQty());
                myPage.setProductId(orderMaked.getProductId());
                // view 레파지 토리에 save
